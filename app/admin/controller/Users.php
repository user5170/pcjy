...
class Users extends Common{
    ...
    public function index(){
        if(request()->isPost()){
            $region_info = db('region')->column('id,name');
            $key=input('post.key');
            ...
            foreach ($list['data'] as $k=>$v){
                ...
                if(!empty($v['province'])){
                    $list['data'][$k]['addresss'] = $region_info[$v['province']]."-".$region_info[$v['city']]."-".$region_info[$v['district']]."-".$v['address'];
                }
                if($v['is_real'] == 1){
                    $list['data'][$k]['is_real'] = '<span style="color:#5FB878">已实名</span>';
                }else{
                    $list['data'][$k]['is_real'] = '<span style="color:#FF5722">未实名</span>';
                }
            }
    ...
    public function approve(){
        if(request()->isPost()){
            ...
            $pageSize =input('limit')?input('limit'):config('pageSize');
            $where = [];
            $where['i.uid|u.username'] = array('like',"%".$key."%");
            $list=db('user_info')->alias('i')
                ->join('users u','u.id = i.uid','left')
                ->field('i.*,u.username')
                ->where($where)
                ->order('i.w_time desc')
                ->paginate(array('list_rows'=>$pageSize,'page'=>$page))
    ...
    public function check(){
        $type = input('type');
        $id = input('pid');
        $yz = db('user_info')->where('id',$id)->find();
        $save = [];
        if($type == 'yes' && $yz){
            $save['status'] = 1;
        }elseif($type == 'no' && $yz){
        ...
        $rs = db('user_info')->where('id',$id)->update($save);
        if($rs){
            if($type == 'yes'){
                $save = [];
                $save['is_real'] = 1;
                $rs2 = db('users')->where('id',$yz['uid'])->update($save);
            }else{
                $save = [];
                $save['is_real'] = 0;
                $rs2 = db('users')->where('id',$yz['uid'])->update($save);
    ...
    public function sign(){
        if(request()->isPost()){
            ...
            $pageSize =input('limit')?input('limit'):config('pageSize');
            $where = [];
            $where['c.uid|u.username'] = array('like',"%".$key."%");
            $list=db('clock')->alias('c')
